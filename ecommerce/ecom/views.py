...
# Credentials Entered check if user is customer or merchant
...
# admin view the Review
@login_required(login_url='adminlogin')
def view_Review_view(request):
    Reviews=models.Review.objects.all().order_by('-id')
    return render(request,'ecom/view_Review.html',{'Reviews':Reviews})
...
def cart_view(request):
    ...
    if 'product_ids' in request.COOKIES:
        ...
        if product_ids != "":
            ...
            #for total p_price shown in cart
            for p in products:
                total=total+p.p_price
...
def remove_from_cart_view(request,pk):
    ...
    if 'product_ids' in request.COOKIES:
        ...
        products=models.Product.objects.all().filter(id__in = product_id_in_cart)
        #for total p_price shown in cart after removing product
        for p in products:
            total=total+p.p_price
...
def send_Review_view(request):
    ReviewForm=forms.ReviewForm()
    if request.method == 'POST':
        ReviewForm = forms.ReviewForm(request.POST)
        if ReviewForm.is_valid():
            ReviewForm.save()
            return render(request, 'ecom/Review_sent.html')
    return render(request, 'ecom/send_Review.html', {'ReviewForm':ReviewForm})
...
def download_invoice_view(request,orderID,productID):
    ...
    mydict={
        ...
        'productp_price':product.p_price,
